...
export default function DirectoryPage() {
  ...
  const filteredKtvs = allKtvs.filter((ktv) => {
    return (
      (filters.search === '' || ktv.name.toLowerCase().includes(filters.search.toLowerCase())) &&
      (filters.district === '' || ktv.district === filters.district) &&
      (filters.type === '' || ktv.type === filters.type) &&
      (filters.services === '' || ktv.services.includes(filters.services))
    );
  });
